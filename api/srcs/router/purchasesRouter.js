...
class PurchasesRouter extends Router {
	...
	buildAddressString(prefix, a) {
		...
		var postal =
			city || code ? 'Code Postal : ' + code + ' ' + city + CR : ''
	...
	attach() {
		...
		this.router.post('/buy', async (req, res) => {
			if (!req.body.cart)
				return res.json({ error: 'missing cart attribute' })
			...
			const emailInformation = this.buildMailRecap(
				purchase,
				total.cart,
				user
			)
		...
		this.router.post('/calculation', (req, res) =>
			this.calculation(req, res)
		)
	}
	...
	createDescriptionPurchase(purchaseInformation, email) {
		return (
			...
			purchaseInformation.cart
				.map(
					(book) =>
						(book.quantity > 1 ? book.quantity + ' x ' : '') +
						book.title
				)
				.join(', ')
		)
		...
		return {
			achat: cart.map((purchase) => {
				...
				return {
					...
					src:
						'https://lichen.com/api/static/image/' +
